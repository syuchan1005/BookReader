...
const DownloadBookInfoDialog = (props: DownloadBookInfoDialogProps) => {
  ...
  const onClickDownload = React.useCallback(async () => {
    ...
    for (let i = 0; i < bookLen; i += 1) {
      ...
      await Promise.all(
        [...Array(book.pages).keys()].map((index) => {
          ...
          const nameA = index
            .toString(10)
            .padStart(book.pages.toString(10).length, '0');
          return fetch(url).then((res) => {
            num += 1;
            setDownloadImages(num);
            bookFolder.file(
              `${nameA}.${defaultStoredImageExtension}`,
              res.blob(),
            );
          });
        }),
    ...
    zip
      .generateAsync({ type: 'blob' }, ({ percent }) =>
        ...
        {!booksLoading && loading && (
          <>
            <LinearProgress
              ...
              color="secondary"
              value={
                (downloadImages / data.bookInfo.books[downloadBooks].pages) *
                100
              }
            />
            <div
              style={{ textAlign: 'center' }}
            >{`${downloadImages} / ${data.bookInfo.books[downloadBooks].pages}`}</div>
            <LinearProgress
              variant="determinate"
              color="secondary"
            ...
            <div
              style={{ textAlign: 'center' }}
            >{`${downloadBooks} / ${data.bookInfo.books.length}`}</div>
            {compressPercent && (
              <div style={{ textAlign: 'center' }}>{`Compressing: ${Math.round(
                compressPercent,
              )}%`}</div>
            )}
          </>
