import { Button, CircularProgress, Dialog, DialogActions, DialogContent, DialogTitle, LinearProgress } from "@mui/material";
...
export const useDownloadBookDialog = (props: Omit<DownloadBookDialogProps, 'open' | 'onClose'>) => {
...
export const DownloadBookDialog = (props: DownloadBookDialogProps) => {
    ...
    const [downloadStatus, setDownloadStatus] = useState<string | undefined>(undefined);
    return (
        <Dialog open={open} onClose={downloadStatus === undefined ? onClose : undefined}>
...
const downloadBook = async (
    ...
    await Promise.all(
        [...Array(totalPageCount).keys()].map((pageIndex) => {
            ...
            const name = pageIndex.toString(10).padStart(totalPageCount.toString(10).length, '0');
            return fetch(url).then(async (res) => {
                downloadedImageCount += 1;
                onProgress(`Downloading images: ${downloadedImageCount} / ${totalPageCount}`);
