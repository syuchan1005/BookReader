...
export const urlToImageData = (url: string): Promise<ImageData> =>
...
export const calcPadding = (
  ...
  const isWhite = (width: number, height: number) => {
    ...
    /* isWhite (R G B (A)) */
    return (
      255 - imageData.data[dataIndex] <= threshold &&
  ...
  const isSameColor = (w1: number, y1: number, w2: number, y2: number) => {
    ...
    return (
      Math.abs(imageData.data[pixelIndex1] - imageData.data[pixelIndex2]) <=
        threshold &&
      Math.abs(imageData.data[pixelIndex1] - imageData.data[pixelIndex2 + 1]) <=
        threshold &&
      Math.abs(imageData.data[pixelIndex1] - imageData.data[pixelIndex2 + 2]) <=
        threshold
    );
  };
  ...
  const compareLeft = useCompareWithWhite
    ? isWhite
    : (w: number, h: number) =>
        isSameColor(horizontalOffset - 1, verticalOffset - 1, w, h);
  leftWidth: for (
    let w = horizontalOffset;
    w < imageData.width - horizontalOffset;
    w += 1
  ) {
    for (
      let h = verticalOffset;
      h < imageData.height - verticalOffset;
      h += 1
    ) {
      if (!compareLeft(w, h)) {
        left = w;
  ...
  rightWidth: for (
    let w = imageData.width - 1 - horizontalOffset;
    w >= horizontalOffset;
    w -= 1
  ) {
    for (
...
const CalcImagePaddingDialog = (props: CalcImagePaddingDialogProps) => {
  ...
  const onDetectClick = React.useCallback(async () => {
    ...
    const { left: l, right: r } = calcPadding(
      rawImageData,
      threshold,
      verticalOffset,
      horizontalOffset,
  ...
  return (
    <Dialog open={open} onClose={onClose}>
      ...
      >
        ...
        />
        <Button fullWidth onClick={onDetectClick}>
          Detect & Preview
        </Button>
        <div
